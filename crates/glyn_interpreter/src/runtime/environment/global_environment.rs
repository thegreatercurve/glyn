...
impl GlobalEnvironment {
    ...
    pub(crate) fn has_binding(
        ...
        if DeclEnvironment::has_binding(env_addr.clone(), name)? {
            return Ok(true);
        }

        // 3. Let ObjRec be envRec.[[ObjectRecord]].
        // 4. Return ? ObjRec.HasBinding(N).
        ObjEnvironment::has_binding(env_addr, name)
    }
    ...
    pub(crate) fn create_mutable_binding(
        ...
        DeclEnvironment::create_mutable_binding(env_addr, name, deletable)
    }
    ...
    pub(crate) fn create_immutable_binding(
        ...
        DeclEnvironment::create_immutable_binding(env_addr, name, strict)
    }
    ...
    pub(crate) fn initialize_binding(
        ...
        if DeclEnvironment::has_binding(env_addr.clone(), &name)? {
            ...
            return DeclEnvironment::initialize_binding(env_addr, name, value);
        }

        // 3. Assert: If the binding exists, it must be in the Object Environment Record.
        debug_assert!(ObjEnvironment::has_binding(env_addr.clone(), &name)?);

        // 4. Let ObjRec be envRec.[[ObjectRecord]].
        // 5. Return ? ObjRec.InitializeBinding(N, V).
        ObjEnvironment::initialize_binding(env_addr, name, value)
    }
    ...
    pub(crate) fn set_mutable_binding(
        ...
        if DeclEnvironment::has_binding(env_addr.clone(), &name)? {
            ...
            return DeclEnvironment::set_mutable_binding(env_addr, name, value, strict);
        }

        // 3. Let ObjRec be envRec.[[ObjectRecord]].
        // 4. Return ? ObjRec.SetMutableBinding(N, V, S).
        ObjEnvironment::set_mutable_binding(env_addr, name, value, strict)
    }
    ...
    pub(crate) fn get_binding_value(
        ...
        if DeclEnvironment::has_binding(env_addr.clone(), name)? {
            ...
            return DeclEnvironment::get_binding_value(env_addr, name, strict);
        }

        // 3. Let ObjRec be envRec.[[ObjectRecord]].
        // 4. Return ? ObjRec.GetBindingValue(N, S).
        ObjEnvironment::get_binding_value(env_addr, name, strict)
    }
    ...
    pub(crate) fn delete_binding(
        ...
        if DeclEnvironment::has_binding(env_addr.clone(), name)? {
            ...
            return DeclEnvironment::delete_binding(env_addr.clone(), name);
        }

        // 3. Let ObjRec be envRec.[[ObjectRecord]].

        // 4. Let globalObject be ObjRec.[[BindingObject]].
        let global_object = env_addr.borrow().obj_env().binding_object();
        ...
        if existing_prop {
            ...
            return ObjEnvironment::delete_binding(env_addr, name);
        }
    ...
    pub(crate) fn has_lexical_declaration(
        ...
        DeclEnvironment::has_binding(env_addr, name)
    }
}
